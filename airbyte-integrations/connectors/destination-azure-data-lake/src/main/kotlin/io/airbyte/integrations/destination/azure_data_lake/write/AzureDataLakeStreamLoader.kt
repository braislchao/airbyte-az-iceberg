...
package io.airbyte.integrations.destination.azure_data_lake.write
...
class AzureDataLakeStreamLoader(
    private val icebergConfiguration: AzureDataLakeConfiguration,
    ...
    private val streamStateStore: StreamStateStore<AzureDataLakeStreamState>,
) : StreamLoader {
    ...
    private fun computeIncomingSchema(withIdentifierFields: Boolean) =
        icebergUtil.toIcebergSchema(stream = stream).let { schema ->
            // Transform the schema to use the mapped (sanitized) column names.
            transformSchemaWithMappedNames(schema, withIdentifierFields)
        }

    /**
     * Transforms an Iceberg schema to use mapped column names. This keeps the Iceberg schema in
     * sync with the sanitized column names produced by the table schema mapper (alphanumeric +
     * underscore only).
     */
    private fun transformSchemaWithMappedNames(
        ...
        // Don't set identifier fields during table creation - some catalogs reassign field IDs.
    ...
    override suspend fun start() {
        val properties =
            azureDataLakeCatalogUtil.toCatalogProperties(config = icebergConfiguration)
        val catalog = icebergUtil.createCatalog(DEFAULT_CATALOG_NAME, properties)
        azureDataLakeCatalogUtil.createNamespace(stream.mappedDescriptor, catalog)
        table =
        ...
        // Reconcile identifier fields after the catalog creates the table, in case the catalog
        // reassigned field IDs.
        val primaryKeyNames =
    ...
    private fun computeOrExecuteSchemaUpdate() =
        icebergTableSynchronizer.maybeApplySchemaChanges(
            ...
            // Generic REST catalogs handle delete+add of a column with the same name in a single
            // schema transaction (unlike BigLake), so no separate commits are required.
            requireSeparateCommitsForColumnReplace = false,
        )
}
